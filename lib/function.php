...
//list($message_modal, $notreFichier) =
 // upload_fichier($directory_upload_files,'file',["application","image"])

function upload_fichier($directory, $methode, $typeFile ) {
    $resultat = false;
    $nameFichier= '';
    $fileNameNew = '';
    $fileType = '';
    ...
    if (isset($_FILES['notreFichier'])) {
        //paramètres du fichier file
        $file = $_FILES['notreFichier'];

        $fileName = $_FILES['notreFichier']['name'];
        $fileTmpName = $_FILES['notreFichier']['tmp_name'];
        $fileSize = $_FILES['notreFichier']['size'];
        $fileError = $_FILES['notreFichier']['error'];
        $fileType = $_FILES['notreFichier']['type'];


        $extToMime_types = array(
            '.3dm' => 'x-world/x-3dmf',
            '.3dmf' => 'x-world/x-3dmf',
        ...
        //ce code permet de gérer les extensions du fichier
        $fileExt = explode('.', $fileName);
        $fileActualExt = strtolower(end($fileExt));
        $allowedfiles = array('.pdf', '.docx', '.jpg', '.jpeg', '.gif',
            '.png', '.bmp');
        $content = mime_content_type($fileTmpName);
        ...
        // number if success
        // false for fail
        if (array_search($content2[0], $typeFile) !== false) {
            $extension = array_search($content, $extToMime_types);
            $result = array_search($extension, $allowedfiles);
            //conditions pour upload de fichier
            if ($fileError === 0) {
                if ($fileSize < 15000000) {
                    ...
                    if ($methode == 'blob') {
                        ...
                        $resultat = file_get_contents($fileTmpName);

                    } else {
                        $resultat = move_uploaded_file($fileTmpName,
                            $fileDestination);
                    }


                    //affichage selon les resultats ou erreurs
                    if ($resultat) {
                        $modal = "Transfert réussi";


                    } else {

                        $modal = "desole ca ne marche pas";
                    }

                } else {
                    $modal = "Le fichier est trop volumineux";
                }

            } else {
                $modal = "Impossible de télécharger le fichier";
            }
        } else {
            $modal = "Extension non valide !";
        }

    } else {
        $modal = "Pas de fichier.";
    }
    return array($modal, $fileNameNew, $resultat, $fileType);
